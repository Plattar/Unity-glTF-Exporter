...
public class GlTF_Parameters : GlTF_Writer {
	...
	public void bakeAndPopulate(AnimationClip clip, int bakingFramerate, string root, string target)
	{
		...
		Transform targetObject = GameObject.Find(target).transform;
		retrieveCurvesFromClip(clip, target, ref translationCurves, ref rotationCurves, ref scaleCurves);
		...
		// TIME
		GlTF_Accessor timeAccessor = new GlTF_Accessor(target + "TimeAccessor", GlTF_Accessor.Type.SCALAR, GlTF_Accessor.ComponentType.FLOAT);
		timeAccessor.bufferView = GlTF_Writer.floatBufferView;
		GlTF_Writer.accessors.Add(timeAccessor);

		GlTF_Accessor translationAccessor = new GlTF_Accessor(target + "TranslationAccessor_" + GlTF_Writer.accessors.Count, GlTF_Accessor.Type.VEC3, GlTF_Accessor.ComponentType.FLOAT);
		translationAccessor.bufferView = GlTF_Writer.vec3BufferView;
		GlTF_Writer.accessors.Add(translationAccessor);

		GlTF_Accessor rotationAccessor = new GlTF_Accessor(target + "RotationAccessor_" + GlTF_Writer.accessors.Count, GlTF_Accessor.Type.VEC4, GlTF_Accessor.ComponentType.FLOAT);
		rotationAccessor.bufferView = GlTF_Writer.vec4BufferView;
		GlTF_Writer.accessors.Add(rotationAccessor);

		GlTF_Accessor scaleAccessor = new GlTF_Accessor(target + "ScaleAccessor_" + GlTF_Writer.accessors.Count, GlTF_Accessor.Type.VEC3, GlTF_Accessor.ComponentType.FLOAT);
		scaleAccessor.bufferView = GlTF_Writer.vec3BufferView;
		GlTF_Writer.accessors.Add(scaleAccessor);
		...
		paramAccessors.Add(target + "_time", timeAccessor);
		paramAccessors.Add(target + "_translation", translationAccessor);
		paramAccessors.Add(target + "_rotation", rotationAccessor);
		paramAccessors.Add(target + "_scale", scaleAccessor);
	}
	public override void Write()
	{
		...
		{
			CommaNL();
			Indent(); jsonWriter.Write("\"" + param.Key + "\": \"" + param.Value.name + "\"");
		}
